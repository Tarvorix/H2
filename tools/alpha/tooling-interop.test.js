...
describe('alpha tooling interop', () => {
    it('builds a selfplay sample from a preselected macro action without rerunning alpha search', () => {
        ...
        const actions = generateMacroActions({ state, actedUnitIds: new Set() }, 0, {
            timeBudgetMs: 20,
        ...
        const sample = createAlphaSelfPlaySample(state, 0, {
            timeBudgetMs: 20,
    ...
    it('imports existing engine selfplay manifests into alpha distill shards', () => {
        ...
        const replay = createReplayArtifact(initialState, [{ command: { type: 'endSubPhase' }, actingPlayerIndex: 0 }], [], {
    ...
    it('runs alpha selfplay directly from a candidate model file', () => {
        ...
        fs.writeFileSync(candidateModelPath, `${JSON.stringify(serializeAlphaModel(createFreshAlphaModel('alpha-model-file-test', {
...
//# sourceMappingURL=tooling-interop.test.js.map