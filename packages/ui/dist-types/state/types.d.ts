...
import type { ModelShape, LOSResult, CoherencyResult, MovementEnvelopeResult, TemplateShape, Scenario } from '@hh/geometry';
import type { GhostTrail } from '../overlays/ghostTrailOverlay';
export type InteractionMode = 'select' | 'los' | 'distance' | 'coherency' | 'movement' | 'blast' | 'template' | 'vehicleFacing' | 'terrainEdit';
...
export type DebugVisualizerAction = {
    ...
    type: 'SET_LOS_MODEL_A';
    modelId: string;
} | {
    type: 'SET_LOS_MODEL_B';
    modelId: string;
} | {
    type: 'CLEAR_LOS';
} | {
    ...
    type: 'SET_TEMPLATE_ORIGIN';
    origin: Position;
} | {
    type: 'SET_TEMPLATE_DIRECTION';
    direction: number;
} | {
    type: 'CLEAR_TEMPLATE';
} | {
    type: 'SET_MOVEMENT_MODEL';
    modelId: string;
} | {
    type: 'SET_MOVEMENT_DISTANCE';
    distance: number;
} | {
    type: 'CLEAR_MOVEMENT';
} | {
    ...
    partial: Partial<TerrainEditorState>;
} | {
    type: 'TERRAIN_DRAG_START';
    worldPos: Position;
} | {
    type: 'TERRAIN_DRAG_MOVE';
    worldPos: Position;
} | {
